...
class Router(val config: NoCConfig, val routingPolicy: RoutingPolicy) extends Module {
  ...
  val packetRoutes = RegInit(VecInit(Seq.fill(config.portNum)(0.U(config.portWidth.W))))

  for (port <- 0 until config.portNum) {
    val flit = inputBuffers(port).io.out.bits
    val hasFlit = inputBuffers(port).io.out.valid

    // Only a head carries src/dst fields. Body and tail flits must retain the
    // route selected by the packet head.
    val headRoute = routingPolicy.route(io.routerId, flit.dstId)
    routeDecisions(port) := Mux(flit.isHead, headRoute, packetRoutes(port))
    when(hasFlit && flit.isHead) {
      packetRoutes(port) := headRoute
    }
  }
