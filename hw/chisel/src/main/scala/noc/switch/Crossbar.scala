...
class Crossbar(config: NoCConfig, numInputs: Int, numOutputs: Int) extends SwitchFabric(config, numInputs, numOutputs) {
  ...
  val outputLocked = RegInit(VecInit(Seq.fill(numOutputs)(false.B)))
  val lockedInput = Reg(Vec(numOutputs, UInt(log2Ceil(numInputs).W)))
  ...
  for (outIdx <- 0 until numOutputs) {
    ...
    for (inIdx <- 0 until numInputs) {
      ...
      val lockAllows = !outputLocked(outIdx) || lockedInput(outIdx) === inIdx.U
      selectedInputs(inIdx).valid := io.in(inIdx).valid && selected && lockAllows
    ...
    // Wormhole packet lock: once a multi-flit head wins an output, retain
    // that input until its tail is transferred. This prevents packet flits
    // from interleaving on the next router's physical input.
    when(arbiters(outIdx).io.out.fire) {
      when(outputLocked(outIdx)) {
        when(arbiters(outIdx).io.out.bits.isTail) {
          outputLocked(outIdx) := false.B
        }
      }.elsewhen(arbiters(outIdx).io.out.bits.isHead &&
                 !arbiters(outIdx).io.out.bits.isTail) {
        outputLocked(outIdx) := true.B
        lockedInput(outIdx) := arbiters(outIdx).io.chosen
      }
    }
  }
  ...
  for (inIdx <- 0 until numInputs) {
    val readySignals = (0 until numOutputs).map { outIdx =>
      val selected = io.select(inIdx) === outIdx.U
      val lockAllows = !outputLocked(outIdx) || lockedInput(outIdx) === inIdx.U
      Mux(selected && lockAllows, arbiters(outIdx).io.in(inIdx).ready, false.B)
    }
